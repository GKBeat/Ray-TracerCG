...
public class BoundingBox {
  ...
  public boolean intersect(Ray ray) {
    if (this.contains(ray.pointAt(ray.tMin))) return true;
    if (this.contains(ray.pointAt(ray.tMax))) return true;
    ...
    return (tmax >= tmin && tmin >= ray.tMin && tmin <= ray.tMax);
  }
  ...
  public Point center() {
    return Vector.interpolate(max, min, 0.5);
  }

  public BoundingBox scale(double factor) {
    Point c = center();
    return new BoundingBox(
        add(multiply(factor, subtract(min, c)), c), add(multiply(factor, subtract(max, c)), c));
  }
